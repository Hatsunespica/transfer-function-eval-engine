...
namespace Evaluation {
    ...
    public:
        TransferFunctionResult(bool isSound = false, bool isExact = false,
                               bool isSolved = false, size_t distance = 0,
                               size_t soundDistance = 0)
                : soundCases(isSound), exactCases(isExact), distance(distance),
                  soundDistance(soundDistance) {
            unsolvedExactCases = isSolved ? isExact : 0;
        }

        void addResult(bool isSound, bool isExact, bool isSolved, size_t distance,
                       size_t soundDistance) {
            soundCases += isSound;
            exactCases += isExact;
    ...
    public:
        ...
        void addIthResult(size_t i, bool isSound, bool isExact, bool isSolved,
                          size_t distance, size_t soundDistance) {
            transferFunctionResults[i].addResult(isSound, isExact, isSolved, distance,
                                                 soundDistance);
        }
